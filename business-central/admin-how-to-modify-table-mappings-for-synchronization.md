...
Les conflits de données peuvent facilement se produire lorsque les applications métier échangent des données de manière continue. Par exemple, quelqu’un peut supprimer ou modifier un enregistrement dans l’une des applications, ou les deux. Pour réduire le nombre de conflits que vous devrez résoudre manuellement, vous pouvez spécifier des stratégies de résolution et [!INCLUDE[d365fin](includes/d365fin_md.md)] résoudra automatiquement les conflits selon les règles des stratégies.

Les mappages de table d’intégration incluent des règles qui contrôlent la façon dont les tâches travaux de synchronisation synchronisent les enregistrements. Sur la page **Mappage de tables d’intégration**, dans les colonnes **Résoudre les conflits de suppression** et **Résoudre les conflits de mise à jour**, vous pouvez spécifier comment [!INCLUDE[d365fin](includes/d365fin_md.md)] résoudra les conflits qui se produisent parce que les enregistrements ont été supprimés dans les tables de l’une ou l’autre application métier, ou mis à jour dans les deux. 

Dans la colonne **Résoudre les conflits de suppression**, vous pouvez choisir que [!INCLUDE[d365fin](includes/d365fin_md.md)] restaure automatiquement les enregistrements supprimés, supprime le couplage entre les enregistrements ou ne fasse rien. Si vous ne faites rien, vous devez résoudre manuellement les conflits. 

Dans la colonne **Résoudre les conflits de mise à jour**, vous pouvez choisir que [!INCLUDE[d365fin](includes/d365fin_md.md)] envoie automatiquement une mise à jour des données à la table d’intégration lors de l’envoi de données à [!INCLUDE[d365fin](includes/cds_long_md.md)], obtienne une mise à jour des données à partir de la table d’intégration lors de l’obtention de données à partir de [!INCLUDE[d365fin](includes/cds_long_md.md)] ou ne fasse rien. Si vous ne faites rien, vous devez résoudre manuellement les conflits.
