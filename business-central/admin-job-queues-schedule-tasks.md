...
3. Choisissez le champ **Code catégorie de la file d’attente des travaux**, puis spécifiez le code **SALESPOST**.
    ...
    La prochaine fois que vous publiez des documents de vente, [!INCLUDE [prodshort](includes/prodshort.md)] crée automatiquement une entrée de file d’attente des travaux pour chaque document et exécute les travaux en arrière-plan, un par un.
...
5. Vérifiez sur la page **Écritures journal file d’attente des travaux** si la commande vente a été validée avec succès. Pour plus d’informations, voir [Pour afficher le statut ou les erreurs dans la fille d’attente](admin-job-queues-schedule-tasks.md#to-view-status-or-errors-in-the-job-queue).
